<template>
    <div class="drag">
        <div className="cart-container">
            <h2>Panier</h2>
            <div v-if="$store.state.Features.cart.length < 0">
                <div className="cart-empty">
                    <p>Panier Vide</p>
                    <div className="start-shopping">
                        <router-link to="/EventsHomeCart">Tous les Evenment</router-link>
                        <span>Réserver vos Tickets</span>
                    </div>
                </div>
            </div>
            <div v-else>
                <div class="titles">
                    ...
                    <h3 class="quantity">Quantité</h3>
                    <h3 class="total">Total</h3>
                </div>
                <div class="cart-items">
                    <div v-for="(c) of $store.state.Features.cart" :key="c.product.id">

                        <div class="cart-item" key={cartItem.id}>
                            <div class="cart-product">
                                <img :src="c.product.imagefilm" :alt="c.product.nomfilm" />
                                <div>
                                    <h3>{{ c.product.nomfilm }}</h3>
                                    <p> Producteur :
                                     {{ getProducerName(c.product.producerID).substr(0, 80) }}
                                    </p>
...
const removeFromCart = (item) => {
    store.commit("Features/removeFromCart", item);
}
const plus = (item) => {
    Produits.value.map((pro) => {
        if (pro.id == item.product.id) {
            if (item.qty < pro.nbplaces) {
                store.commit("Features/plusCart", item);
            }
            else
                alert("Tous les places sont réservés")
        }
    });
}
const minus = (item) => {
    ...
    if (item.qty < 1) {
        ...
        store.commit("Features/minusCart", item);
}
...
const getProducerName = (producerID) => {
    ...
    return producer ? producer.nompreproducer : 'Non défini';
};
