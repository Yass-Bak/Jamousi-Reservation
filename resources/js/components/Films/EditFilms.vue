        ...
        <div class="card flex justify-content-center">
            ...
            <select multiple class="form-control" id="acteurs" v-model="film.acteurIDs">
                <option v-for="acteur in Acteurs" :key="acteur.id" :value="acteur.id">{{ acteur.nompreacteur }}</option>
                    ...
                    <hr>
                    <button type="submit" className="btn btn-outline-success" @click="modifierfilms">
...
const handleFilePondInit = async () => {
    if (film.value.imagefilm) {

        myFiles.value = [
            {
                source: film.value.imagefilm,
                options: { type: 'local' }
            }
        ]
    }
